  ...
  <div class="mx-auto flex max-w-sm flex-col gap-6 py-12">
    ...
    <form class="flex flex-col gap-4" @submit.prevent="onSubmit">
      <div class="flex flex-col gap-1">
        <label class="text-sm font-medium text-slate-700">I am a...</label>
        <div class="flex gap-3">
          <label
            class="flex flex-1 cursor-pointer items-center justify-center rounded-md border px-3 py-2 text-sm"
            :class="role === UserRole.Candidate ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-300 text-slate-700'"
          >
          ...
          <label
            class="flex flex-1 cursor-pointer items-center justify-center rounded-md border px-3 py-2 text-sm"
            :class="role === UserRole.Employer ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-300 text-slate-700'"
          >
    ...
    <p class="text-center text-sm text-slate-600">
      Already have an account? <NuxtLink to="/login" class="font-medium text-slate-900 underline">Log in</NuxtLink>
    </p>
  </div>
</template>
