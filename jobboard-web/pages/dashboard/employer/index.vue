...
<template>
  <div class="flex flex-col gap-6">
    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Employer Dashboard</h1>

    <div v-if="status === 'pending'" class="text-slate-500 dark:text-slate-400">Loading…</div>
    ...
    <div v-else-if="!company" class="rounded-lg border border-slate-200 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
      <p class="mb-4 text-slate-600 dark:text-slate-400">You haven't set up your company profile yet.</p>
      <NuxtLink
    ...
    <div v-else class="flex flex-col gap-4">
      <div class="rounded-lg border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
        <div class="flex items-start justify-between">
          <div>
            ...
            <p class="text-sm text-slate-600 dark:text-slate-400">{{ company.location || 'No location set' }}</p>
          </div>
          <NuxtLink to="/dashboard/employer/company" class="text-sm text-slate-600 underline dark:text-slate-400">Edit</NuxtLink>
        </div>
      </div>
      ...
      >
        <h2 class="font-semibold text-slate-900 dark:text-slate-100">My Job Listings</h2>
        <p class="text-sm text-slate-600 dark:text-slate-400">View, edit, and manage your postings.</p>
      </NuxtLink>
    </div>
  </div>
</template>
