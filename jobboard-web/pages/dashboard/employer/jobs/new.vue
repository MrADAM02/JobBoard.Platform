...
<template>
  <div class="mx-auto flex max-w-lg flex-col gap-6 py-6">
    <h1 class="text-2xl font-bold text-slate-900">Post a job</h1>

    <div v-if="!company" class="rounded-lg border border-amber-200 bg-amber-50 p-6 text-amber-800">
    ...
    <form v-else class="flex flex-col gap-4" @submit.prevent="onSubmit">
      ...
      <div class="flex flex-col gap-1">
        ...
        <input id="tags" v-model="tags" type="text" placeholder="dotnet, vue, remote" class="rounded-md border border-slate-300 px-3 py-2 text-sm">
      </div>
      <label class="flex items-center gap-2 text-sm text-slate-700">
        <input v-model="publishImmediately" type="checkbox" class="h-4 w-4 rounded border-slate-300">
