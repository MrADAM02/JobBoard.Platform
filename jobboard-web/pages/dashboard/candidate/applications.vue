...
<template>
  <div class="flex flex-col gap-6">
    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">My Applications</h1>
      ...
      <li v-for="app in applications" :key="app.id" class="rounded-lg border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
        <div class="flex items-center justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-slate-600 dark:text-slate-400">{{ app.companyName }} &middot; Applied {{ new Date(app.appliedAt).toLocaleDateString() }}</p>
          </div>
