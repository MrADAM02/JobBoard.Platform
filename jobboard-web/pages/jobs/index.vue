...
<template>
  <div class="flex flex-col gap-8">
    <div>
      <h1 class="text-2xl font-bold text-slate-900">Browse Jobs</h1>
    ...
    <form
      class="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-5"
    ...
    >
      <input
        ...
        class="rounded-md border border-slate-300 px-3 py-2 text-sm lg:col-span-2"
      >
      <input
        v-model="location"
        type="text"
      ...
      >
      <select v-model="jobType" class="rounded-md border border-slate-300 px-3 py-2 text-sm">
      ...
      <input
        ...
        class="rounded-md border border-slate-300 px-3 py-2 text-sm"
      >
      <label class="flex items-center gap-2 text-sm text-slate-700 sm:col-span-2 lg:col-span-1">
        <input v-model="remoteOnly" type="checkbox" class="h-4 w-4 rounded border-slate-300">
        Remote only
      </label>
      <button
        type="submit"
        class="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700 sm:col-span-2 lg:col-span-1"
      >
        Search
      </button>
    </form>

    <div v-if="status === 'pending'" class="py-16 text-center text-slate-500">
      Loading jobs&hellip;
    </div>

    <div v-else-if="error" class="rounded-lg border border-red-200 bg-red-50 p-6 text-center text-red-700">
      Something went wrong loading job listings. Please try again.
    </div>

    <div v-else-if="!data?.items.length" class="rounded-lg border border-slate-200 bg-white p-10 text-center text-slate-500">
    ...
    <ul v-else class="flex flex-col gap-3">
      <li v-for="job in data.items" :key="job.id">
        <NuxtLink
          :to="`/jobs/${job.id}`"
          class="block rounded-lg border border-slate-200 bg-white p-5 transition hover:border-slate-400 hover:shadow-sm"
        >
          <div class="flex items-start justify-between gap-4">
            <div>
              <h2 class="font-semibold text-slate-900">{{ job.title }}</h2>
              <p class="text-sm text-slate-600">{{ job.companyName }} &middot; {{ job.location }}</p>
            </div>
            <span
              v-if="job.isRemote"
              class="whitespace-nowrap rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-700"
            >
              Remote
            </span>
          </div>
          <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
            <span class="rounded-full bg-slate-100 px-2.5 py-1">{{ JobTypeLabels[job.jobType] }}</span>
            <span v-if="job.salaryMin || job.salaryMax" class="rounded-full bg-slate-100 px-2.5 py-1">
    ...
    <div v-if="data && data.totalPages > 1" class="flex items-center justify-center gap-3 pt-2">
      ...
      <span class="text-sm text-slate-600">Page {{ data.pageNumber }} of {{ data.totalPages }}</span>
      <button
