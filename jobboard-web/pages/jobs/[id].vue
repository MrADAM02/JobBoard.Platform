...
// Schema.org's employmentType vocabulary is a fixed spec (FULL_TIME, etc.) -
// never localized, regardless of the active locale.
...
// route.fullPath is already locale-prefix-aware (e.g. /ar/jobs/{id}), so this
// stays correct under prefix_except_default without manual path building.
const pageUrl = `${useRequestURL().origin}${route.fullPath}`

// The template text is translated; the interpolated job data (title, company,
// location, description) stays exactly as the employer typed it - per the
// static-UI-only translation scope, job content is never auto-translated.
const description = computed(() =>
  t('jobs.detail.seoDescription', {
    title: job.value!.title,
    company: job.value!.companyName,
    location: job.value!.location,
    excerpt: job.value!.description.slice(0, 150)
  })
)
...
useSeoMeta({
  title: () => t('jobs.detail.seoTitle', { title: job.value!.title, company: job.value!.companyName }),
  description,
  ogTitle: () => t('jobs.detail.ogTitle', { title: job.value!.title, company: job.value!.companyName }),
