...
<template>
  <article v-if="job" class="flex flex-col gap-6">
    <NuxtLink to="/jobs" class="text-sm text-slate-500 hover:text-slate-700">&larr; Back to all jobs</NuxtLink>
    ...
    <header class="rounded-lg border border-slate-200 bg-white p-6">
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-slate-600">{{ job.companyName }} &middot; {{ job.location }}</p>
        </div>
        <span
          v-if="job.isRemote"
      ...
      <div class="mt-4 flex flex-wrap gap-2 text-xs text-slate-500">
        ...
        <span v-if="salaryRange" class="rounded-full bg-slate-100 px-2.5 py-1">{{ salaryRange }}</span>
        <span v-if="job.tags" class="rounded-full bg-slate-100 px-2.5 py-1">{{ job.tags }}</span>
      </div>
    </header>

    <section class="rounded-lg border border-slate-200 bg-white p-6">
      <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Description</h2>
      <p class="whitespace-pre-line text-slate-700">{{ job.description }}</p>
    </section>

    <section v-if="job.companyWebsite" class="rounded-lg border border-slate-200 bg-white p-6">
      <h2 class="mb-2 text-sm font-semibold uppercase tracking-wide text-slate-500">About {{ job.companyName }}</h2>
      <a :href="job.companyWebsite" target="_blank" rel="noopener" class="text-sm text-slate-700 underline">
        {{ job.companyWebsite }}
      </a>
    </section>
